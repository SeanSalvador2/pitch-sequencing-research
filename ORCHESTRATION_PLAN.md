...
| WS5b docs+notebook | 2026-07-14 | Artifacts verified (41-cell notebook valid/compiles/0 outputs; D44 story through all docs; honest verdict in the abstract; canonical MDP/OPE references). Awaiting final no-regression suite exit before commit. | pending |
| D45 | WS6 runs ALL FIVE views with **capacity-matched architectures**: C = static-context MLP; U = order-invariant mean-pooled token encoder + static head; L1 = GRU truncated to the last token; O = full GRU over the ordered PA sequence (sequences.py tensors: family/outcome embeddings + float channels incl. dvelo/dloc); OM = O + matchup features in the static head. Selection and outcome1 targets (outcome conditions on the current action embedding per D22) | the deep ablation must vary information, not capacity |
| D46 | WS6 training discipline: compact GRU (embed ≈16, hidden ≈64, ≤2 layers), early stopping on val, seeded/deterministic in tests; full training runnable-but-not-run in Phase 1. Phase 2 ships BOTH paths: local CPU (honest runtime estimate) and a self-contained Colab T4 notebook; ROCm/DirectML caveat noted with Colab recommended. torch lives in a `[deep]` extra with import guards | Sean's execution contract; D9 fulfilled |
| D47 | WS6 falsification = the shared battery via a torch model_factory adapter (D17), plus the synth gates: null world selection GRU-O must detect the planted habit (beat L1) and outcome must stay quiet (D21); positive world outcome GRU-O must beat U/L1 (the dvelo channel carries the mechanism directly). Report whether the learned model rediscovers WS2's explicit motifs (lightweight probing) | the representation-learning question WS6 exists to answer |
